...
define(function (require, exports, module) {
  ...
  Engine.on('click', function () {
    if (!inTransition) {
      ...
      if (!toggle)
        surfaces[0].surface.state.setSize(newsize, {curve: myEasing, duration: 300}, function(){
          surfaces[1].surface.state.setSize(newsize, {curve: myEasing, duration: 300});
          flex.setRatios(initialRatios, {curve: myEasing, duration: 500}, function(){ inTransition = false; });
        });
      else
        surfaces[1].surface.state.setSize(newsize, {curve: myEasing, duration: 300}, function(){
          surfaces[0].surface.state.setSize(newsize, {curve: myEasing, duration: 300});
          flex.setRatios(initialRatios, {curve: myEasing, duration: 500}, function(){ inTransition = false; });
        });
      //surfaces[1].surface.state.setSize(newsize, {curve: myEasing, duration: 200});
      flex.setRatios(initialRatios, {curve: myEasing, duration: 500}, function(){ inTransition = false; });
      toggle = !toggle;
    }
  });
